...
from keras.applications import xception, vgg16, efficientnet_v2, inception_resnet_v2, resnet_v2, inception_v3, vgg19, mobilenet_v2, densenet, mobilenet_v3, nasnet
...
class InceptionV3_FE:
    def __init__(self) -> None:
        base_model = inception_v3.InceptionV3()  # Create InceptionV3 model

        # Create model based on InceptionV3 model above
        # With input is the same as InceptionV3 model
        # And output results from avg_pool layer of InceptionV3 model
        # (None, 299, 299, 3) --> (None, 2048)
        self.model = Model(inputs=base_model.input, outputs=base_model.get_layer('avg_pool').output)
    ...
    def extract(self, image) -> np.ndarray:
        ...
        array = img_to_array(image)  # Image to np.array with shape (299, 299, 3)
        array = np.expand_dims(array, axis=0)  # (299, 299, 3) --> (1, 299, 299, 3)
        array = inception_v3.preprocess_input(array)  # Subtracting average values for each pixel
...
class MobileNetV2_FE:
    def __init__(self) -> None:
        base_model = mobilenet_v2.MobileNetV2()  # Create MobileNetV2 model

        # Create model based on MobileNetV2 model above
        # With input is the same as MobileNetV2 model
        # And output results from global_average_pooling2d layer of MobileNetV2 model
    ...
    def extract(self, image) -> np.ndarray:
        ...
        array = img_to_array(image)  # Image to np.array with shape (224, 224, 3)
        array = np.expand_dims(array, axis=0)  # (224, 224, 3) --> (1, 224, 224, 3)
        array = mobilenet_v2.preprocess_input(array)  # Subtracting average values for each pixel
